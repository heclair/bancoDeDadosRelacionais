...
CREATE VIEW v_depcidade AS SELECT d.nome AS nome_departamento, c.nome AS nome_cidade FROM tbl_departamentos d INNER JOIN tbl_cidades c ON d.cod_cidade = c.cod_cidade;
...
CREATE OR REPLACE VIEW v_depcidadehouston AS SELECT d.* FROM tbl_departamentos d INNER JOIN tbl_cidades c ON d.cod_cidade = c.cod_cidade WHERE c.nome = 'Houston';
...
CREATE VIEW v_opsalario AS SELECT SUM(salario) AS soma_salarios, AVG(salario) AS media_salarios FROM tbl_empregados;
...
CREATE VIEW vw_empregados_salarial AS SELECT nome, salario FROM tbl_empregados WHERE salario > 5500.00;
